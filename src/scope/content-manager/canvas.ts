...
export default class CanvasManager implements ContentManager {
  ...
  protected createNewNode(
    ...
    const siblingsRight = siblings?.reduce(
    ...
    const x =
      siblingsRight != null ? siblingsRight + NEW_NOTE_MARGIN : parentNode.x;
    const y =
      (priorSibling
        ? priorSibling.y
        : parentNode.y + parentNode.height + NEW_NOTE_MARGIN) +
      height * 0.5;
    ...
    this.addConnection(
      ...
      { fromOrTo: "from", side: "bottom", node: parentNode },
      { fromOrTo: "to", side: "top", node: newNode }
    );
  ...
  protected addConnection(
    connectionID: string,
    fromConnection: CanvasEdgeIntermediate,
    toConnection: CanvasEdgeIntermediate
  ) {
    ...
    this.canvas.importData({
      edges: [
        ...
        {
          id: connectionID,
          fromNode: fromConnection.node.id,
          fromSide: fromConnection.side,
          toNode: toConnection.node.id,
          toSide: toConnection.side,
        },
      ],
      nodes: data.nodes,
    });
  ...
  async getSelections(): Promise<string[]> {
    // @ts-ignore
    return (
      (await this.getTextSelectedItems())
        .map((e) => e?.rawText)
        .filter(Boolean) || []
    );
  }
...
const generateRandomHexString = (len: number) =>
