...
export class PluginManager {
  ...
  constructor(options?: Partial<PluginManagerOptions>) {
    ...
    this.githubRegistry = new GithubRegistryClient(
      this.options.githubAuthentication
    );
    this.bitbucketRegistry = new BitbucketRegistryClient(
      this.options.bitbucketAuthentication
    );
  }
  ...
  private async installFromGithubLockFree(
    repository: string
  ): Promise<IPluginInfo> {
  ...
  private async installFromBitbucketLockFree(
    repository: string
  ): Promise<IPluginInfo> {
  ...
  private async installFromCodeLockFree(
    name: string,
    code: string,
    version = "0.0.0"
  ): Promise<IPluginInfo> {
